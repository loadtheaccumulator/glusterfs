...
from glustolibs.gluster.volume_ops import (setup_vol, get_volume_info,
                                           get_volume_status)
from distaf.util import testcase, tc
from glustolibs.gluster.ganesha import (setup_nfs_ganesha,
...
class SetupNfsGaneshaVol(GlusterBaseClass):
    ...
    def setup(self):
        ...
        if ret:
            g.log.info("setup of ganesha for %s node is successfull"
                           % self.no_of_ganesha_nodes)
        else:
            g.log.error("setup of ganesha for %s node is unsuccessfull"
                            % self.no_of_ganesha_nodes)
            return False
        ...
        if ret is None:
            return False
        ret, out, err = g.run(self.mnode, "showmount -e localhost")
        if ret != 0:
            return False
        ...
        if ret != 0:
            g.log.error("Mounting Volume %s failed on %s:%s" %
    ...
    def teardown(self, teardown_ganesha_setup=False):
        ...
        """
        umount_volume(g.config['clients'][0]['host'], self.mountpoint)
        ret, out, err = g.run(g.config['clients'][0]['host'], "rm -rf %s/*" % self.mountpoint)
        time.sleep(5)
        if ret != 0:
            g.log.error("rm -rf command failed on the mountpoint %s"
                            % self.mountpoint)
            return False
