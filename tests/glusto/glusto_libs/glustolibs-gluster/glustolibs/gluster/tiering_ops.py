...
import glustolibs.gluster.volume_ops
...
def add_peer_nodes_to_cluster(peers, mnode=None):
    ...
    if not ret:
        g.log.error("Unable to do peer probe on peer machines")
        return False
...
def get_files_from_hot_tier(volname):
    ...
    files = []
    subvols = glustolibs.gluster.volume_ops.get_subvols(volname)
...
def get_tier_promote_frequency(volname):
    ...
    vol_options = glustolibs.gluster.volume_ops.get_volume_option(volname)
...
def set_tier_promote_frequency(volname, value):
    ...
    if not glustolibs.gluster.volume_ops.set_volume_option(volname,
                                                           options=option):
