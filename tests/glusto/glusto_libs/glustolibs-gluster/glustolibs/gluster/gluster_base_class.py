...
from glustolibs.gluster.gluster_init import env_setup_servers
...
class GlusterBaseClass():
    ...
    def _create_volume(self):
        ...
        if self.voltype == 'distribute':
            ...
            g.log.error("The volume type is not present")
            return False
        ret = setup_vol(self.volname, self.mnode, dist, rep, dispd, red,
                        stripe, trans, servers=self.servers)
        if not ret:
            g.log.error("Unable to setup volume %s", self.volname)
            return False
        return True
