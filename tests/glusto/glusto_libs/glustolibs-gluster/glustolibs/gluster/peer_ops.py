    ...
    for server in servers:
        if server not in nodes_in_pool_list:
            ...
            if (ret != 0 or
                ...
                g.log.info("Successfully peer probed the node '%s'.",
                               server)
...
def get_peer_status(mnode=None):
    ...
    ret, out, _ = g.run(mnode, "gluster peer status --xml", verbose=False)
    if ret != 0:
...
def get_pool_list(mnode=None):
    ...
    ret, out, _ = g.run(mnode, "gluster pool list --xml", verbose=False)
    if ret != 0:
