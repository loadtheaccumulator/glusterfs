...
from glustolibs.gluster.volume_ops import start_volume
...
def ctdb_firewall_settings(servers=None):
    ...
    _rc = True
    for server in servers:
        ret, out, _ = g.run(server, "cat /etc/redhat-release")
        if not ret:
            match = re.search('.*release (\d+).*', out)
            if match is None:
                g.log.error("Failed to find OS version")
                return False
            if match.group(1) == '7':
                ret, out, err = g.run(server, "service firewalld start")
                if ret != 0:
                    g.log.error("Failed to start firewalld")
                ret, _, _ = g.run(server, "firewall-cmd --zone=public "
                                   "--add-service=samba --add-service="
                                   "glusterfs")
                if ret != 0:
                    g.log.error("Failed to set firewall zone for samba")
                    _rc = False
                ret, _, _ = g.run(server, "firewall-cmd --zone=public "
                                   "--add-service=samba --add-service="
                                   "glusterfs --permanent")
                ...
                ret, _, _ = g.run(server, "firewall-cmd --zone=public "
                                   "--add-port=4379/tcp")
                if ret != 0:
                    g.log.error("Failed to add port for samba")
                    _rc = False
                ret, _, _ = g.run(server, "firewall-cmd --zone=public "
                                   "--add-port=4379/tcp --permanent")
                if ret != 0:
                    g.log.error("Failed to add port for samba permanently")
                    _rc = False
    return _rc
...
def update_smb_conf(servers=None):
    ...
    _rc = True
    for server in servers:
        ret, _, _ = g.run(server, "grep 'clustering=yes' %s" % file_path)
        if ret == 0:
            g.log.info("%s file is already edited")
            continue
        ret, _, _ = g.run(server, "sed -i '/^\[global\]/a clustering=yes' %s"
                           % file_path)
        if ret != 0:
            g.log.error("failed to edit %s file on %s"
                            % (file_path, server))
            _rc = False
...
def update_hook_scripts(servers=None):
    ...
    if not isinstance(servers, list):
        servers = [servers]
    for server in servers:
        ret, _, _ = g.run(server, "sed -i s/'META=\"all\"'/'META=\"ctdb\"'/g"
                           " %s %s" % (file1_path, file2_path))
        if ret != 0:
            g.log.error("failed to edit %s hook-script on %s"
                            % (file_path, server))
            _rc = False
...
def create_ctdb_nodes_file(servers=None):
    ...
    if not isinstance(servers, list):
        servers = [servers]
    for server in servers:
        ret, out, _ = g.run(server, "hostname -i")
        if ret != 0:
            g.log.error("failed to get ip of %s" % server)
            _rc = False
        ctdb_ips_list.append(out)
    for server in servers:
        conn = g.rpyc_get_connection(server, "root")
        try:
...
def create_ctdb_meta_volume(mnode=None, servers=None, meta_volname=None):
    ...
    for i, server in enumerate(servers):
        brick_list = (brick_list + "%s:%s/ctdb_brick%s "
                      % (server, brick_path, i))
    ret, _, _ = g.run(mnode, "gluster volume create %s replica %s %s force "
                       "--mode=script" % (meta_volname, replica_count,
                                          brick_list))
    if ret != 0:
        g.log.error("failed to create meta volume ctdb")
        return False
...
def check_if_gluster_lock_mount_exists(servers=None):
    ...
    _rc = True
    for server in servers:
        ret, _, _ = g.run(server, "cat /proc/mounts | grep '/gluster/lock'")
        if ret != 0:
            g.log.error("/gluster/lock mount does not exist on %s"
                            % server)
            _rc = False
...
def check_if_ctdb_file_exists(servers=None):
    ...
    for server in servers:
        ...
        if not conn.modules.os.path.isfile(file_path):
            g.log.error("%s file not found on %s" % (file_path, server))
            _rc = False
# FIXME:        conn.close()
...
def start_ctdb_service(servers=None):
    ...
    _rc = True
    for server in servers:
        ret, _, _ = g.run(server, "service ctdb start")
        if ret != 0:
            g.log.error("failed to start the ctdb service on %s" % server)
            _rc = False
...
def verify_ctdb_status(mnode=None):
    ...
    if mnode is None:
        mnode = (tc.global_config['gluster']['cluster_config']
                 ['smb']['ctdb_servers'][0]['host'])
    _rc = True
    ret, out, _ = g.run(mnode, "cat /etc/ctdb/nodes")
    if ret != 0:
        g.log.error("failed to get the details of /etc/ctdb/nodes file")
        return False
    servers_list = out.strip().split("\n")
    ret, out, _ = g.run(mnode, "ctdb status | grep pnn")
    if ret != 0:
        g.log.error("failed to get the details of ctdb status")
        return False
    ...
    for status in status_list:
        for server in servers_list:
            if server in status:
                if "OK" in status:
                    g.log.info("ctdb status for %s is OK" % server)
                else:
                    g.log.error("ctdb status for %s is not OK" % server)
                    _rc = False
...
def ctdb_gluster_setup(mnode=None, servers=None, meta_volname=None):
    ...
    if ret:
        g.log.info("firewall settings successfull for ctdb setup")
    else:
        g.log.error("firewall settings failed for ctdb setup")
        return False
    ...
    if ret:
        g.log.info("successfully created ctdb meta volume")
    else:
        g.log.error("failed to create ctdb meta volume")
        return False
    g.run(mnode, "gluster v info %s" % meta_volname)

    # 4. update the ctdb hook scripts
    ret = update_hook_scripts(servers[:])
    if ret:
        g.log.info("successfully updated the hook scripts on all servers")
    else:
        g.log.error("failed to update the hook scripts on "
                        "one or more servers")
        return False
    ...
    if ret:
        g.log.info("successfully updated the smb.conf file on all servers")
    else:
        g.log.error("failed to update the smb.conf file on "
                        "one or more servers")
        return False
    ...
    if ret:
        g.log.info("successfully started the meta volume")
        g.run(mnode, "gluster v status ctdb")
    else:
        g.log.error("failed to start the meta volume")
        return False
    time.sleep(20)
    # 6.b check if /gluster/lock mount exists on all servers
    ret = check_if_gluster_lock_mount_exists(servers[:])
    if ret:
        g.log.info("/gluster/lock mount exists on all servers")
    else:
        return False
    ...
    if ret:
        g.log.info("/etc/sysconfig/ctdb file exists on all servers")
    else:
        return False
    ...
    if ret:
        g.log.info("successfully created /etc/ctdb/nodes file on "
                       "all servers")
    else:
        g.log.error("failed to create /etc/ctdb/nodes file on "
                        "one or more servers")
        return False
    ...
    if ret:
        g.log.info("successfully created /etc/ctdb/public_addresses file "
                       "on all servers")
    else:
        g.log.error("failed to create /etc/ctdb/public_addresses file "
    ...
    if ret:
        g.log.info("successfully started ctdb service on all servers")
    else:
        return False
    time.sleep(360)
    ...
    if ret:
        g.log.info("ctdb status is correct")
    else:
        g.log.error("ctdb status is incorrect")
        return False
...
def stop_ctdb_service(servers=None):
    ...
    _rc = True
    for server in servers:
        ret, _, _ = g.run(server, "service ctdb stop")
        if ret != 0:
            g.log.error("failed to stop the ctdb service on %s" % server)
            _rc = False
