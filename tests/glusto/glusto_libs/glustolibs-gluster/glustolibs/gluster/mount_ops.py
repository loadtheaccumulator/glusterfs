...
def is_mounted(volname, mpoint, mserver, mclient):
    ...
    if ret == 0:
        g.log.debug("Volume %s is mounted at %s:%s" % (volname,
                                                           mclient,
                                                           mpoint))
        return True
    else:
...
def mount_volume(volname, mtype='glusterfs', mpoint='/mnt/glusterfs',
    ...
    global tc
    if mserver == '':
        mserver = g.config['servers'][0]['host']
    if mclient == '':
        mclient = g.config['clients'][0]['host']
    if options != '':
        options = "-o %s" % options
    ...
    if mtype == 'cifs':
        from glustolibs.gluster.samba_ops import setup_samba_service
        ...
        if not setup_samba_service(volname, mserver, smbuser, smbpasswd):
            g.log.error("Failed to setup samba service %s" % mserver)
            return (1, '', '')
