...
def create_volume(volname, mnode=None, dist=1, rep=1, stripe=1, trans='tcp',
    ...
    Kwargs:
        mnode(str): server on which command has to be execeuted,
            defaults to g.config['servers'][0]['host']
    ...
    if disp != 1 and dispd != 1:
        g.log.error("volume can't have both disperse and disperse-data")
        return (-1, None, None)
    if disp != 1:
        dispc = int(disp)
    elif dispd != 1:
    ...
    from glustolibs.gluster.lib_utils import form_bricks_path
    ...
    ret = g.run(mnode, "gluster volume create %s %s %s %s %s %s %s %s "
...
def setup_vol(volname, mnode=None, dist=1, rep=1, dispd=1, red=1,
    ...
    if volinfo is not None and volname in volinfo.keys():
        g.log.debug("volume %s already exists in %s. Returning..." \
                % (volname, servers[0]))
        return True
    ret = env_setup_servers(servers=servers)
    if not ret:
        g.log.error("Formatting backend bricks failed. Aborting...")
        return False
    ret = start_glusterd(servers)
    if not ret:
        g.log.error("glusterd did not start in at least one server")
        return False
    time.sleep(5)
    ret = peer_probe_servers(servers[1:], mnode=mnode)
    if not ret:
        g.log.error("Unable to peer probe one or more machines")
        return False
    if rep != 1 and dispd != 1:
        g.log.warning("Both replica count and disperse count is specified")
        g.log.warning("Ignoring the disperse and using the replica count")
        dispd = 1
        red = 1
...
def get_volume_status(volname='all', service='', options='', mnode=None):
    ...
    if volume_list is None:
        g.log.error("No volumes exists in the gluster")
        return None
...
def get_volume_info(volname='all', mnode=None):
    ...
    ret, out, _ = g.run(mnode, cmd, verbose=False)
    if ret != 0:
