...
from glustolibs.gluster.volume_ops import start_volume, stop_volume
import re
import time
...
def get_snap_status(mnode=None):
    ...
    ret, out, _ = g.run(mnode, "gluster snapshot status --xml", verbose=False)
    if ret != 0:
...
def get_snap_info(mnode=None):
    ...
    ret, out, _ = g.run(mnode, "gluster snapshot info --xml", verbose=False)
    if ret != 0:
...
def get_snap_list(mnode=None):
    ...
    ret, out, _ = g.run(mnode, "gluster snapshot list --xml", verbose=False)
    if ret != 0:
...
def get_snap_config(volname=None, mnode=None):
    ...
    ret, out, _ = g.run(mnode, "gluster snapshot config --xml", verbose=False)
    if ret != 0:
