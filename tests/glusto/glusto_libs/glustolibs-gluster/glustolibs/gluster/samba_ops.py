...
def setup_samba_service(volname, mnode, username, smbpasswd):
    ...
    Returns:
        bool:
            ...
            False: On Failure
    Example:
        setup_samba_service("testvol", "g.config['servers'][0]['host']", "Bond", "redhat")
    """
    try:
        if tc.global_flag['SAMBA_SETUP_DONE'] == True:
            g.log.error("samba is already setup for the cluster")
            return True
    except KeyError:
        pass
    ...
    ret, _ = g.run_serial(glstrd_edit_cmd)
    if not ret:
        g.log.error("Unable to edit glusterd in all servers")
        return False
    ...
    g.log.info("chkconfig smb on successfull")
    ...
    ret, _, _ = g.run(mnode, smbpasswd_cmd)
    if ret != 0:
        g.log.error("Unable to set the smb password")
        return False

    time.sleep(20)

    ret, out, err = g.run(mnode, "gluster volume set %s "
    ...
    ret, out, err = g.run(mnode, "gluster volume set %s stat-prefetch off"
                           % volname)
    if ret != 0:
    ...
    set_volume_cmd = ("gluster volume set %s storage.batch-fsync-delay-usec 0"
                      % volname)
    ret, out, err = g.run(mnode, set_volume_cmd)
    if ret != 0:
        g.log.error("Failed to set the volume option storage"
                        "batch-fsync-delay-usec 0")
        return False

    smb_cmd = "smbclient -L localhost -U " + username + "%" + smbpasswd

    ret, out, err = g.run(mnode, smb_cmd + "| grep -i -Fw gluster-%s "
                           % volname)
    if ret != 0:
        g.log.error("unable to find share entry")
        return False
    g.log.info("Share entry present")
