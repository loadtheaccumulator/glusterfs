...
def append_string_to_file(mnode, filename, str_to_add_in_file):
    ...
    """
    try:
        conn = g.rpyc_get_connection(mnode, 'root')
        if conn == -1:
...
def calculate_checksum(file_list, chksum_type='sha256sum', mnode=None):
    ...
    for line in ret[1].split('\n')[:-1]:
        ...
        if match is None:
            g.log.error("checksum output is not in"
                            "expected format")
            return None
    ...
    for each_attr in ret[1].split('\n\n')[:-1]:
        for line in each_attr.split('\n'):
            if line.startswith('#'):
                ...
                if match is None:
                    g.log.error("getfattr output is not in "
                                    "expected format")
                    return None
                key = "/" + match.group(1)
                attr_dict[key] = {}
...
def get_servers_bricks_dict(servers):
    ...
    Example:
        get_servers_bricks_dict(g.config['servers'])
    """
...
def get_servers_used_bricks_dict(servers, mnode):
    ...
    Example:
        get_servers_used_bricks_dict(g.config['servers'][:], g.config['servers'][0]['host'])
    """
    if not isinstance(servers, list):
        servers = [servers]
...
def get_servers_unused_bricks_dict(servers, mnode):
    ...
    Example:
        get_servers_unused_bricks_dict(g.config['servers'], g.config['servers'][0]['host'])
    """
    if not isinstance(servers, list):
        servers = [servers]
...
def form_bricks_path(number_of_bricks, servers, mnode, volname):
    ...
    Example:
        form_bricks_path(6, g.config['servers'], g.config['servers'](0), "testvol")
    """
    if not isinstance(servers, list):
        servers = [servers]
    dict_index = 0
